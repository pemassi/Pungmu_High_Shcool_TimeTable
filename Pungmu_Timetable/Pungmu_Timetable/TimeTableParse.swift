...
class TimeTable_Teacher
{
    ...
    init(_ mode:Mode)
    {
        ...
        //Update Data
        Data_Save.setValue(table, forKey: Data_Key.Time_TeacherTimeTable)
        Data_Save.setValue(subject, forKey: Data_Key.Time_Subject)
        Data_Save.setValue(teacher, forKey: Data_Key.Time_Teacher)
...
{
    static func GetSchoolCode(target:String) -> (SchoolCode:[String], LiveName:[String], SchoolName:[String])
    {
        let Data = Webkit.GetWebRespone("http://115.20.145.150:4080/_hourdat?sc2=\(target.toEUC_KR())")!
        let Data_List = Data.split("\"학교검색\":[[")[1].replace("]]}","").split("],[")
        
        var _SchoolCode = [String](count: Data_List.count, repeatedValue: "")
        var _LiveName = [String](count: Data_List.count, repeatedValue: "")
        var _SchoolName = [String](count: Data_List.count, repeatedValue: "")
        
        for var i = 0; i < Data_List.count ; i++
        {
            var j  = Data_List[i].split(",")
            _SchoolCode[i] = j[0]
            _LiveName[i] = j[1].replace("\"","")
            _SchoolName[i] = j[2].replace("\"", "")
        }
        
        return (_SchoolCode,_LiveName,_SchoolName)
    }
}
